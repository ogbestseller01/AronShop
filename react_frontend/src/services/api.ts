...
// @ts-ignore
import appConfig from '../config'; // <-- Make sure this import exists
 ...
 * ============================================================
 *
 * Automatically attaches the Bearer token to authenticated
 * requests.
 */
...
export const authApi = {
  ...
  resendVerification: (data: { email: string }) =>
    api.post<ApiResponse<ResendVerificationResponse>>(
      '/v1/auth/resend-verification',
      data
    ),

  forgotPassword: (data: { email: string }) =>
    api.post<ApiResponse<ForgotPasswordResponse>>(
...
export const roleApi = {
  ...
  assignPermissions: (id: string, permissionIds: string[]) =>
    api.post<ApiResponse<Role>>(`/v1/roles/${id}/permissions/assign`, {
      permissions: permissionIds,
    }),

  syncPermissions: (id: string, permissionIds: string[]) =>
    api.post<ApiResponse<Role>>(`/v1/roles/${id}/permissions/sync`, {
      permissions: permissionIds,
    }),

  revokePermission: (id: string, permissionId: string) =>
    api.post<ApiResponse<Role>>(`/v1/roles/${id}/permissions/revoke`, {
      permission_id: permissionId,
    }),

  getDropdown: () => api.get<ApiResponse<Role[]>>('/v1/roles/dropdown'),

  getStats: () => api.get<ApiResponse<any>>('/v1/roles/stats'),
};
...
export const permissionApi = {
  index: (params?: { search?: string; per_page?: number }) =>
    api.get<ApiResponse<PaginatedData<Permission>>>('/v1/permissions', {
      params,
    }),
...
export const userApi = {
  ...
  resetPassword: (
    ...
    api.post<ApiResponse<null>>(`/v1/users/${id}/reset-password`, {
      password,
  ...
  stats: () => api.get<ApiResponse<any>>('/v1/users/stats'),

  dropdown: (params?: { search?: string; active_only?: boolean }) =>
    api.get<ApiResponse<{ id: string; name: string }[]>>('/v1/users/dropdown', {
      params,
    }),
};
...
export const shopApi = {
  ...
  dropdown: (params?: { active_only?: boolean }) =>
    api.get<ApiResponse<{ id: string; label: string }[]>>(
      '/v5/shops/dropdown',
      { params }
    ),
};
...
export const auditApi = {
  index: (params?: AuditTrailFilters & { page?: number }) =>
    api.get<ApiResponse<PaginatedData<AuditTrail>>>('/v1/audit-trails', {
      params,
    }),
  ...
  getStats: (params?: Omit<AuditTrailFilters, 'per_page' | 'page'>) =>
    api.get<ApiResponse<AuditTrailStats>>('/v1/audit-trails/stats', {
      params,
    }),

  exportCsv: (params?: AuditTrailFilters) =>
    api.get<Blob>('/v1/audit-trails/export/csv', {
      params,
      responseType: 'blob',
    }),

  exportExcel: (params?: AuditTrailFilters) =>
    api.get<Blob>('/v1/audit-trails/export/excel', {
      params,
      responseType: 'blob',
    }),

  exportPdf: (params?: AuditTrailFilters) =>
    api.get<Blob>('/v1/audit-trails/export/pdf', {
      params,
      responseType: 'blob',
    }),
};
...
export const failedLoginApi = {
  index: (params?: FailedLoginFilters & { page?: number }) =>
    api.get<
      ApiResponse<{
        data: PaginatedData<FailedLoginAttempt>;
        stats: FailedLoginStats;
      }>
    >('/v1/auth', { params }),

  clear: (params?: { email?: string; ip_address?: string }) =>
    api.delete<ApiResponse<{ deleted_count: number }>>('/v1/auth/clear', {
      data: params,
    }),

  block: (ip_address: string) =>
    api.post<ApiResponse<null>>('/v1/auth/block', {
      ip_address,
    }),

  unblock: (ip_address: string) =>
    api.post<ApiResponse<null>>('/v1/auth/unblock', {
      ip_address,
    }),
};
...
export const companyApi = {
  ...
  trashed: (params?: { search?: string; per_page?: number }) =>
    api.get<ApiResponse<PaginatedData<Company>>>('/v18/companies/trashed', {
      params,
    }),
...
export const categoryApi = {
  index: (params?: ProductCategoryFilters & { page?: number }) =>
    api.get<ApiResponse<PaginatedData<ProductCategory>>>(
  ...
  update: (id: string, data: Partial<ProductCategoryFormData>) =>
    api.put<ApiResponse<ProductCategory>>(
      `/v2/product-categories/${id}`,
      data
    ),
  ...
  activate: (id: string) =>
    api.patch<ApiResponse<ProductCategory>>(
      `/v2/product-categories/${id}/activate`
    ),

  deactivate: (id: string) =>
    api.patch<ApiResponse<ProductCategory>>(
      `/v2/product-categories/${id}/deactivate`
    ),

  toggleStatus: (id: string) =>
    api.patch<ApiResponse<ProductCategory>>(
      `/v2/product-categories/${id}/toggle-status`
    ),
  ...
  dropdown: (params?: { search?: string; include_inactive?: boolean }) =>
    api.get<
      ...
      >
    >('/v2/product-categories/dropdown', { params }),
};
...
export const reportApi = {
  ...
  exportSales: (params?: SalesReportFilters) =>
    api.get<Blob>('/v19/reports/sales/export', {
      params,
      responseType: 'blob',
    }),

  analysis: () =>
    api.get<ApiResponse<any>>('/v19/reports/analysis'),

  agentsWithSales: (params?: any) =>
    api.get<ApiResponse<any>>('/v19/reports/agents', { params }),

  shopsWithSales: (params?: any) =>
    api.get<ApiResponse<any>>('/v19/reports/shops', { params }),
};
